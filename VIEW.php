...
$sql = "SELECT Name, Phone, Email, Message FROM submit";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr> 
                        ...
                        <td>{$row['Email']}</td>
                        <td>{$row['Message']}</td>
                        <td><a href='EDIT.php?id={$row['id']}&name={$row['Name']}&phone={$row['Phone']}&email={$row['Email']}&message={$row['Message']}'>Edit</a></td>
